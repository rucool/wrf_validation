...
def main(args):
    ...
    if not os.path.exists(sst_file):
        if not print_all:
            print('ALERT: SST file '+sst_file+' not found.')
        exit()
    ...
    if len(goes_file)==0:
        if print_all:
            ...
            print('ALERT: Original GOES file for time '+pd.to_datetime(sst.time.values[0]).strftime('%Y%m%dT%H%M')+' not found in '+goes_dir)
    else:
        ...
        if len(goes_file)>0:
            ...
            if goesnan==1:
                if print_all:
                    ...
                    print('ALERT: No GOES data included')
            else:
    ...
    if rtgnan>0:
        if not print_all:
            print('ALERT: WRF SST composite contains missing data')
